...
from database import engine, get_db, Product, ProductKey, Order, User
...
#   ALLOWED_ORIGINS=https://jt29hub.vercel.app
...
# Phone number for TrueMoney redeem — set via env variable
...
def topup_truemoney(req: TopupRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Allow TEST- prefix for local/dev testing
    ...
    try:
        ...
        if res.status_code != 200:
            if "cloudflare" in res.text.lower() or "<html" in res.text.lower():
                raise HTTPException(
                    status_code=400,
                    detail="TrueMoney system is temporarily blocking automated requests (Cloudflare restriction).",
                )
            try:
                error_msg = res.json().get("status", {}).get("message", "Invalid voucher or network error")
                raise HTTPException(status_code=400, detail=error_msg)
            except Exception:
                raise HTTPException(status_code=400, detail=f"TrueMoney API response error (HTTP {res.status_code})")
...
def add_product(req: AddProductRequest, admin_user: User = Depends(get_admin_user), db: Session = Depends(get_db)):
    p = Product(name=req.name, category=req.category, description=req.description, price=req.price)
    db.add(p)
    db.commit()
    db.refresh(p)
    return p
